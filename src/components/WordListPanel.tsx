...
export default function WordListPanel({ result, methodId }: Props) {
  ...
  function getWordList(s: { bandId: string; wordList: string[] }): string[] {
    if (isCoca && s.bandId === 'k21plus') {
      return [...new Set([...s.wordList, ...result.offListWords])].sort();
    }
    return s.wordList;
  }

  function getTypeCount(s: { bandId: string; typeCount: number }): number {
    if (isCoca && s.bandId === 'k21plus') {
      return new Set([...result.bandStats.find(b => b.bandId === 'k21plus')!.wordList, ...result.offListWords]).size;
    }
    return s.typeCount;
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      {result.bandStats.map(s => {
        const words = getWordList(s);
        const count = getTypeCount(s);
        return (
          <div key={s.bandId} className="border-b border-gray-100 last:border-0">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">({count} unique words)</span>
              </span>
      ...
      {!isCoca && result.offListWords.length > 0 && (
        <div>
          <button
            ...
            <div className="px-4 pb-3">
              <div className="flex flex-wrap gap-1.5">
                {result.offListWords.map(w => (
                  <span
                    key={w}
                    className="text-xs px-2 py-0.5 rounded-full bg-white border border-gray-300 font-mono text-gray-600"
                  >
                    {w}
                  </span>
                ))}
